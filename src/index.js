...
// In-memory cache (resets per instance; KV optional later)
let cache = null;

// Helper to fetch flights
async function fetchFlights(params, pageNum, env) {
  try {
...
async function getFlights(env) {
  try {
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      const data = await fetchFlights(params, page + 1, env);
      totalApiCalls++;
    ...
    return {
      ...
      suggestion: 'Check API quota, try 2025-04-16, or contact AviationStack support.',
    };
  }
}
...
export default {
  async fetch(request, env, ctx) {
    const result = await getFlights(env);
